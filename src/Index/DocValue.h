...
#ifndef SRC_INDEX_DOCVALUE_H_
#define SRC_INDEX_DOCVALUE_H_
...
enum class DocValuesType {
   ...
   * and store an additional offset pointer per document to dereference 
   * the shared byte[]. The stored byte[] is presorted and allows access via
   ...
   * access via document id, ordinal and by-value. Values must be 
   * {@code <= 32766} bytes.
   */
  SORTED_SET
};
