...
TEST(AAA, BBB) {
  double d = 123.123;
  int64_t num = NumericUtils::DoubleToSortableLong(d);
  std::cout << "num -> " << num << std::endl;
  char buf[1024];
  NumericUtils::LongToSortableBytes(num, buf, 0);
  for (int i = 0 ; i < 8 ; ++i) {
    std::cout << static_cast<int32_t>(buf[i]) << std::endl;
  }

  int64_t num1 = NumericUtils::SortableBytesToLong(buf, 0); 
  std::cout << "num1 -> " << num1 << std::endl;
  double d1 = NumericUtils::SortableLongToDouble(num1);
